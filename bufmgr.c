			...
			bool *foundPtr)
{
	...
	for (;;)
	{
		...
		//Calling FreeListSpinLockRelease() function here
...
/*
 * Buffer -- make buffer available for replacement.
 *
 * This should be applied only to shared buffers, never local ones.
 *
...
UnpinBuffer(BufferDesc *buf, bool fixOwner)
{
	...
	if (ref->refcount == 0)
	{
		 ...
		 *
		 * Note that the buffer may have already been marked non-accessble
